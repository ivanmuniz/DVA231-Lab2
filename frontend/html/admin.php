			...
			<div id="main_container" class="main-container">
				<div class="message">
					<?php if( isset($_GET['upload_ok']) ) {
							if( $_GET['upload_ok'] == 0 ) {
								if($_GET['status'] == 1 ) { echo "<span class=\"success\">Sorry, only JPG, JPEG, PNG, GIF, MP4, OGV, OGG, AVI, FLV files are allowed.</span>"; }
								elseif($_GET['status'] == 2 ) { echo "<span class=\"error\">Sorry, file already exists.</span>"; }
								elseif($_GET['status'] == 3 ) { echo "<span class=\"error\">Sorry, your file is too large.</span>"; }
								elseif($_GET['status'] == 4 ) { echo "<span class=\"error\">Sorry, there was an error uploading your file.</span>"; }
							} 
							else { echo "success"; }
						}
					?>
				...
				<form id="form_admin" class="form-admin" method="post" action="../../backend/script/admin.php" enctype="multipart/form-data">
					...
					<table>
						...
						<tr>
							<td>media type:</td>
							<td>
								<input type="radio" id="mdia_type_img" name="media_type" value="img" /> image
								&nbsp;&nbsp;&nbsp;
